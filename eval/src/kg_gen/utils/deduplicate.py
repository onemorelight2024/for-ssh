...
class DeduplicateList:
    ...
    def deduplicate(self, items: list[str]) -> list[str]:
        ...
        # Count duplicates: items that were normalized to the same form + semhash duplicates
        normalization_duplicates = self.total_items - len(normalized_items)
        semhash_duplicates = 0
        for item in deduplication_result.selected_with_duplicates:
            semhash_duplicates += len(item.duplicates)
        
        self.duplicate_items = normalization_duplicates + semhash_duplicates
        self.reduction = (self.duplicate_items / self.total_items) * 100 if self.total_items > 0 else 0

        # Map back to original strings using selected_with_duplicates
        # First, handle semhash duplicates
        for item in deduplication_result.selected_with_duplicates:
            canonical_form = item.record
            # Process semhash duplicates for this item
            for duplicate_record, similarity_score in item.duplicates:
                if duplicate_record in self.items_map and canonical_form in self.items_map:
                    # Map the duplicate's original form to the canonical form's original
                    canonical_original = self.items_map[canonical_form]
                    duplicate_original = self.items_map[duplicate_record]
                    self.items_map[duplicate_record] = canonical_original
                    if duplicate_record not in self.duplicates:
                        self.duplicates[duplicate_record] = canonical_form
